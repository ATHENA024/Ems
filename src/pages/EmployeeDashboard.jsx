...
import { formatCurrency, getProfilePhoto } from '../utils/helper';
...
function EmployeeDashboard() {
  ...
  const [photo] = useState(() => getProfilePhoto(user?.email));
  ...
  const profile = empData || { name: user?.name || 'Employee', position: '—', dept: '—', salary: 0, firstName: user?.name?.split(' ')[0] || 'Employee', lastName: user?.name?.split(' ')[1] || 'User' };
    ...
    <div className="edash">
      ...
      <div className="edash-grid-4">
        <div className="card edash-stat">
          <img src={photo || `https://ui-avatars.com/api/?name=${profile.name?.replace(' ', '+')}&background=6366f1&color=fff&size=48`} alt="" style={{ width: 48, height: 48, borderRadius: 12, objectFit: 'cover', flexShrink: 0 }} />
          <div>
